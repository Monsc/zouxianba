...
const TopicPage = () => {
  ...
  const fetchTopicPosts = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await api.get(`/topics/${tag}/posts`, {
        params: {
          page: currentPage,
          limit: 10,
          sort: sortBy,
      ...
      setError(err.message || '获取话题帖子失败');
      toast.error('获取话题帖子失败，请重试');
    } finally {
      setLoading(false);
    }
  };
      ...
      <MainLayout>
        <div className="max-w-4xl mx-auto">
          <ErrorState
            title="获取话题帖子失败"
            description={error}
            action={<Button onClick={fetchTopicPosts}>重试</Button>}
          />
        </div>
      </MainLayout>
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 bg-white rounded-lg shadow-sm">
          <div className="p-4 border-b">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">#{tag}</h1>
        ...
        <LoadingOverlay isLoading={loading}>
          {!Array.isArray(posts) || posts.length === 0 ? (
            <EmptyState
              ...
              action={<Button onClick={() => router.push('/post/create')}>发布动态</Button>}
            />
          ) : (
