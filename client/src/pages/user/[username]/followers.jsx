...
const FollowersPage = () => {
  ...
  const fetchFollowers = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/users/${username}/followers?page=${currentPage}&limit=20`);
      setFollowers(response.data.followers);
      setTotalPages(response.data.totalPages);
    } catch (err) {
      setError(err.message || '加载粉丝列表失败');
          ...
          <div className="divide-y">
            {followers.length > 0 ? (
              followers.map(follower => (
                <div key={follower._id} className="p-6 flex items-center justify-between">
                  ...
                  {currentUser && currentUser._id !== follower._id && (
                    <Button
                      ...
                      onClick={() => handleFollow(follower._id, follower.isFollowing)}
                    >
